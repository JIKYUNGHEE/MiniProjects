...
<script>
	...
	function getIt(data){
		...
		for(var i =0;i<list.length;i++)
		{
			...
			msg += "<td width='15%'>"+nickname+"</td>"+"<td width='65%'>"+content+"</td>"
			
			if( id == "${sessionScope.userVO.nickname}" || "${sessionScope.userVO.type}"=='S')
			{
				msg+="<td>"+"<input id='replyDelete' type='button' value='삭제'/>"+"<input id='reply_no' type='hidden' value="+reply_no+">"+"</td>";
			}
		...
		<div class="bg-faded p-4 my-4">
			...
			<div class="form-group col-lg-12">
				...
				<table class="ajax" style="width: 100%">
					<c:forEach items="${reply }" var="reply">
						<tr>
							<td width="15%">${reply.nickname }</td>
							<td width="65%">${reply.content }</td>
							<td><c:if
									test="${ (userVO.nickname eq reply.id) or (sessionScope.userVO.type eq 'S') }">
									<input id="replyDelete" type="button" value="삭제" />
				...
				<input id="like_no" type="hidden" value="${like.like_no }" /> <input
					id="like_id" type="hidden" value="${like.id }" /> <input
