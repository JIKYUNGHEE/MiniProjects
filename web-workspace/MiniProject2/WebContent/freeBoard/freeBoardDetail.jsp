...
<script>
	...
	/* $("#deletereply_btn").click(function(){
			 	...
			 	for (var i = 0; i < list.length; i++) { 			 	 		 		
	                ...
	                for (var key in list[i]) {	                   
	                    ...
	                    if(key == 'reply_no'){
	                    	...
	                    	output += "<td width='10%'>" + list[i].id + "</td>" ;
	                 ...
	                 if(list[i].id == "${sessionScope.userVO.nickname}" || "${ sessionScope.userVO.type}"=='S'){
	                    	output += "<input type='button' id='deletereply_btn' value='삭제'/></td></tr>"
	                    }
	                 else{
	                	 output += "</td></tr>"
	                 }
		...
		<div  class="w3-border w3-padding">[Comments]</div><br/>
			<div class="w3-border w3-padding">
					...
					<c:forEach items="${ replylist }" var="reply" varStatus="loop">
						<tr>	
							<td width="10%"><input type="hidden" id="reply_id" value="${ reply.id }"/>${ reply.id }</td>						
							...
							<input type="button" id="deletereply_btn" value="삭제"/>
							</c:if>
